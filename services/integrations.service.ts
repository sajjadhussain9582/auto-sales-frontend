...
export const integrationsService = {
  ...
  async getCalendlyEventTypes(): Promise<any> {
    const res = await fetch(`${getApiBaseUrl()}/integrations/calendly/event-types`, {
      headers: getHeaders(),
    });
    if (!res.ok) throw new Error(await readErrorResponse(res));
    return res.json();
  ...
  async disconnect(provider: string): Promise<any> {
    const res = await fetch(`${getApiBaseUrl()}/integrations/${encodeURIComponent(provider)}/disconnect`, {
      method: "POST",
      headers: getHeaders(),
    });
    if (!res.ok) throw new Error(await readErrorResponse(res));
